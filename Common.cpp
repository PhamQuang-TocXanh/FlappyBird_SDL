...
bool loadSound(Mix_Chunk*& jump, Mix_Music*& music, Mix_Chunk*& pass)
{
	...
	music = Mix_LoadMUS("sound/NTDVH.wav");
	if (music == NULL) {
...
bool checkCollision(SDL_Rect collider, SDL_Rect top, SDL_Rect bottom)
{
	...
	if (collider.x > top.x + top.w || collider.w > bottom.x + bottom.w) return false;
	if (collider.y + collider.h < bottom.y && collider.y>top.y + top.h) return false;
...
void Base::Render(int x, int y, SDL_Renderer*& renderer, SDL_Rect* clip, SDL_Rect* des, double angle, SDL_RendererFlip flip)
{
